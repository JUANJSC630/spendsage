...
export function QuickAddFAB() {
  ...
  const isValid = amount.length > 0 && category.length > 0 && description.trim().length > 0;

  const handleSubmit = async () => {
    if (!isValid || isPending) return;
    try {
      await createTransaction({ amount, category, description: description.trim(), date });
        ...
        >
          ...
          <div className="px-6 pb-6 space-y-3.5">
            ...
            <Input
              ...
              className="bg-slate-50 border-0 rounded-2xl h-12 text-sm font-medium text-slate-800 placeholder:text-slate-300 focus-visible:ring-0"
            />
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                style={!isToday && !isYesterday ? { backgroundColor: colorTheme } : {}}
              >
