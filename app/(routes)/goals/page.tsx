...
function EditGoalDialog({
  ...
  goal: import("@/hooks/use-savings-goals").SavingsGoal;
  open: boolean;
  onClose: () => void;
  colorTheme: string;
  symbol: string;
  ...
  const onSubmit = (values: z.infer<typeof editFormSchema>) => {
    const savedAmountNum = values.savedAmount === "" ? "0" : values.savedAmount;
    const targetNum = parseFloat(values.targetAmount);
    const savedNum = parseFloat(savedAmountNum);
    updateGoal(
      {
        id: goal.id,
        name: values.name,
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      type="text"
                      inputMode="numeric"
                      value={formatAmount(form.watch("savedAmount"))}
                      onChange={(e) =>
                        field.onChange(e.target.value.replace(/\D/g, ""))
                      }
                      className="border-slate-200"
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button
                ...
                className="flex-1 h-10 rounded-xl text-white text-sm font-semibold disabled:opacity-40 transition-opacity"
      ...
      { id: goalId, savedAmount: newSaved, isComplete },
      {
        onSuccess: () => {
          toast.success(
            isComplete
              ? "🎉 ¡Meta alcanzada!"
              : `+${symbol}${formatAmount(amt.toString())} abonado`,
          );
          setValue("");
          setOpen(false);
        },
...
export default function GoalsPage() {
  ...
  const [editingGoal, setEditingGoal] = useState<
    import("@/hooks/use-savings-goals").SavingsGoal | null
  >(null);
      ...
      >
        ...
        <motion.div variants={fadeUp} className="mb-6">
          ...
          <p className="text-sm text-slate-400 mt-1">
            {active.length} activa{active.length !== 1 ? "s" : ""} · {symbol}
            {formatAmount(Math.round(totalSaved).toString())} de {symbol}
            {formatAmount(Math.round(totalTarget).toString())} ahorrado
          </p>
        </motion.div>
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          ...
                          placeholder="Ej: Viaje a Europa, Carro..."
                        />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                          ...
                          <div className="flex-1 min-w-0">
                                ...
                                <AlertDialog>
                                  ...
                                  <AlertDialogContent>
                                    <AlertDialogHeader>
                                      ...
                                      <AlertDialogDescription>
                                        Se eliminará{" "}
                                        <strong>{goal.name}</strong> y su
                                        progreso. Esta acción no se puede
                                        deshacer.
                                      </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
                            ...
                            <div className="flex items-center justify-between text-[11px] mb-1.5">
                              <span className="text-slate-500 tabular-nums">
                                <span className="font-semibold text-slate-700">
                                  {symbol}
                                  {formatAmount(Math.round(saved).toString())}
                                </span>{" "}
                                de {symbol}
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                {daysLeft !== null && (
                                  <span
                                    className={`flex items-center gap-1 text-[10px] font-medium ${daysLeft < 30 ? "text-red-500" : "text-slate-400"}`}
                                  >
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-emerald-600 font-medium tabular-nums">
                          {symbol}
                          {formatAmount(goal.targetAmount)} ahorrado
                        </p>
                      </div>
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Se eliminará <strong>{goal.name}</strong> del
                              historial.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
