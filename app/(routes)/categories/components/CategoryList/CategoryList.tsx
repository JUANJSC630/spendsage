...
export function CategoryList({
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    try {
      ...
      if (error.response?.status === 403) {
        toast.error(
          "No se pueden eliminar las categorías por defecto del sistema",
        );
      } else if (error.response?.status === 400) {
        toast.error(
          "No se puede eliminar esta categoría porque está siendo utilizada",
        );
      } else {
  ...
  const handleRestoreCategory = async (categoryId: string) => {
    try {
      ...
      if (error.response?.status === 403) {
        toast.error(
          "No se pueden modificar las categorías por defecto del sistema",
        );
      } else {
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-1 sm:gap-2 mb-0.5 sm:mb-1">
                        ...
                        <Badge
                          className={`text-xs ${typeColors[category.type] || typeColors.other}`}
                        >
                            ...
                            <AlertDialogDescription>
                              {showInactive ? (
                                ...
                                <>
                                  ...
                                  <strong>{category.name}</strong>. Podrás
