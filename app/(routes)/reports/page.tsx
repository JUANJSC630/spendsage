...
export default function ReportsPage() {
  ...
  const { data: aiSummary = [], isFetching: isFetchingAI } = useQuery<string[]>(
    {
      queryKey: ["ai-report-summary", year],
      queryFn: async () => {
        const res = await fetch(`/api/ai/report-summary?year=${year}`);
        if (!res.ok) return [];
        const data = await res.json();
        return (data.summary as string[]).filter(Boolean);
      },
  ...
  // Annual totals
  const { totalIncome, totalExpenses, totalSaved, savingsRate } =
    useMemo(() => {
  ...
  const worstMonth = useMemo(() => {
    ...
    if (!withData.length) return null;
    return withData.reduce((worst, m) =>
      m.expenses > worst.expenses ? m : worst,
    );
  }, [monthlyData]);
  ...
  return (
    ...
    >
      ...
      >
        <div className="rounded-2xl bg-emerald-50 p-4">
          ...
          <p className="text-lg font-bold text-emerald-700 tabular-nums leading-none">
            {symbol}
            {formatAmount(Math.round(totalIncome).toString())}
          </p>
        </div>
        ...
        <div className="rounded-2xl bg-red-50 p-4">
          ...
          <p className="text-lg font-bold text-red-600 tabular-nums leading-none">
            {symbol}
            {formatAmount(Math.round(totalExpenses).toString())}
          </p>
        </div>

        <div
          className={`rounded-2xl p-4 ${totalSaved >= 0 ? "bg-blue-50" : "bg-red-50"}`}
        >
          <div className="flex items-center gap-1.5 mb-2">
            <PiggyBank
              className={`w-3.5 h-3.5 ${totalSaved >= 0 ? "text-blue-400" : "text-red-400"}`}
            />
            <p
              className={`text-[11px] font-medium ${totalSaved >= 0 ? "text-blue-500" : "text-red-500"}`}
            >
              Ahorro anual
            </p>
          </div>
          <p
            className={`text-lg font-bold tabular-nums leading-none ${totalSaved >= 0 ? "text-blue-700" : "text-red-600"}`}
          >
            {totalSaved >= 0 ? "+" : "−"}
            {symbol}
            {formatAmount(Math.abs(Math.round(totalSaved)).toString())}
          </p>
          {totalIncome > 0 && (
            <p
              className={`text-[10px] mt-0.5 font-medium ${totalSaved >= 0 ? "text-blue-400" : "text-red-400"}`}
            >
          ...
          {bestMonth ? (
            <>
              ...
              <p className="text-[10px] text-amber-500 mt-0.5 font-medium tabular-nums">
                +{symbol}
                {formatAmount(Math.round(bestMonth.balance).toString())}
              </p>
            </>
          ) : (
      ...
      {/* AI Annual Summary */}
      {(aiSummary.length > 0 || isFetchingAI) && (
          ...
          {isFetchingAI ? (
            <div className="space-y-2">
              {[85, 70, 90, 60].map((w, i) => (
                <div
                  key={i}
                  className="h-3.5 bg-slate-200 rounded-full animate-pulse"
                  style={{ width: `${w}%` }}
                />
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {aiSummary.map((line, i) => (
                <div
                  key={i}
                  className="flex items-start gap-2.5 bg-white rounded-xl px-3.5 py-2.5"
                >
                  ...
                  <div key={i}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs font-semibold text-slate-700 tabular-nums ml-2 flex-shrink-0">
                        {symbol}
                        {formatAmount(Math.round(cat.amount).toString())}
                      </span>
                    </div>
              ...
              .map((m) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between text-[11px] tabular-nums">
                      <span className="text-emerald-600 font-medium">
                        +{symbol}
                        {formatAmount(Math.round(m.income).toString())}
                      </span>
                      <span className="text-red-500 font-medium">
                        −{symbol}
                        {formatAmount(Math.round(m.expenses).toString())}
                      </span>
                    </div>
                  ...
                  >
                    {m.balance >= 0 ? "+" : "−"}
                    {symbol}
                    {formatAmount(Math.abs(Math.round(m.balance)).toString())}
                  </span>
                </div>
              ))}
            {monthlyData.filter((m) => m.income > 0 || m.expenses > 0)
              .length === 0 && (
              <div className="flex items-center justify-center py-8">
