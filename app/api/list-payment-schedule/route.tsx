...
export async function GET() {
  try {
    ...
    const enriched = lists.map(({ paymentSchedules, ...list }) => {
      ...
      const totalAmount = allItems.reduce((acc, i) => acc + parseFloat(i.amount.replace(/\./g, "")), 0);
      const paidAmount = allItems.filter((i) => i.check).reduce((acc, i) => acc + parseFloat(i.amount.replace(/\./g, "")), 0);
      const progress = totalItems === 0 ? 0 : Math.round((paidItems / totalItems) * 100);
      return { ...list, _stats: { totalItems, paidItems, totalAmount, paidAmount, progress } };
    });
